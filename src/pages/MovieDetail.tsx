...
const MovieDetail = () => {
  ...
  const movieId = id || '0';
  const contentTypeParam = searchParams.get('type'); // 'tv' or 'movie'
  ...
  // 1. Fetch content with correct type handling
  ...
  // 2. Servers Configuration (including reliable Hindi sources)
  const servers = useMemo(() => {
    if (!tmdbId) return [];

    // Standard Embed Paths
    ...
    return [
      ...
      { id: 'hdhub', name: 'HINDI: HDHUB', url: `https://vidsrc.cc/v2/embed/${path}`, tag: 'Hindi' },
      { id: '2embed', name: 'SERVER: 2EMBED', url: `https://www.2embed.cc/embed/${isTV ? `${tmdbId}/${season}/${episode}` : tmdbId}`, tag: 'Multi' },
      { id: 'vidsrc-to', name: 'SERVER: VIDSRC.TO', url: `https://vidsrc.to/embed/${path}`, tag: 'Stable' },
      ...
      { id: 'autoembed', name: 'HINDI: AUTO', url: `https://autoembed.co/${isTV ? 'tv' : 'movie'}/tmdb/${tmdbId}${isTV ? `?s=${season}&e=${episode}` : ''}`, tag: 'Hindi' },
    ];
  }, [tmdbId, isTV, season, episode]);

  // Set default server on load or when season/episode changes
  useEffect(() => {
    if (servers.length > 0) {
      // Preserve the active server type if possible when switching episodes
      const currentServerId = servers.find(s => s.url === selectedStreamUrl)?.id;
      if (currentServerId) {
        const updatedUrl = servers.find(s => s.id === currentServerId)?.url;
        if (updatedUrl) setSelectedStreamUrl(updatedUrl);
      } else if (!selectedStreamUrl) {
        setSelectedStreamUrl(servers[0].url);
      }
    }
  }, [servers]);

  // 3. API Fetching for Bot Links
  useEffect(() => {
  ...
  if (!movie) return <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center p-4 text-center text-white"><h1 className="text-2xl font-bold mb-4">Content not found</h1><Button onClick={() => navigate('/')} className="bg-red-600">Return Home</Button></div>;
        ...
        <div className="max-w-6xl mx-auto space-y-8">
          ...
          <section className="space-y-6">
                    ...
                    <div className="flex gap-4 w-full md:w-auto">
                        <div className="flex-1 md:w-32">
                            ...
                            <Select value={season.toString()} onValueChange={(v) => { setSeason(parseInt(v)); setEpisode(1); }}>
                                <SelectTrigger className="w-full bg-black/40 border-white/10 rounded-xl"><SelectValue placeholder="S1" /></SelectTrigger>
                        ...
                        <div className="flex-1 md:w-32">
                            ...
                            <Select value={episode.toString()} onValueChange={(v) => setEpisode(parseInt(v))}>
                                <SelectTrigger className="w-full bg-black/40 border-white/10 rounded-xl"><SelectValue placeholder="E1" /></SelectTrigger>
                                <SelectContent className="bg-zinc-900 border-white/10 text-white">
            ...
            <div className="bg-zinc-900/30 p-6 rounded-[2.5rem] border border-white/5 space-y-6">
                ...
                <div id="server-grid" className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    ...
                    {/* Bot Servers */}
                    {apiResults.slice(0, 4).map((bot, idx) => (
                        <Button
                            key={`bot-${idx}`}
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Button
                    onClick={() => window.open(`https://vidsrc.me/download/${isTV ? 'tv' : 'movie'}?tmdb=${tmdbId}${isTV ? `&sea=${season}&epi=${episode}` : ''}`, '_blank')}
                    className="h-20 bg-gradient-to-r from-red-600 to-red-800 hover:from-red-500 hover:to-red-700 rounded-3xl shadow-xl transition-all hover:scale-[1.02] group no-underline flex items-center justify-center gap-3 text-white"
                >
                    <Download className="w-6 h-6 group-hover:animate-bounce" />
                    <span className="text-lg font-black italic uppercase">Direct Download (HD)</span>
                </Button>
                <Button
